...
@test "docker-kibana proxies with credentials to Elasticsearch ${KIBANA_VERSION}" {
  web_log="${BATS_TEST_DIRNAME}/web.log"
  ( while echo "$HTTP_RESPONSE_HEAD" '{"version": {"number": "'"${KIBANA_VERSION}"'"}}' | nc -l 456; do : ; done ) > "$web_log" &
