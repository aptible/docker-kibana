...
@test "It should install Kibana 5.1.2" {
...
@test "docker-kibana proxies with credentials to Elasticsearch 5.1" {
  web_log="${BATS_TEST_DIRNAME}/web.log"
  ( while echo "$HTTP_RESPONSE_HEAD" '{"version": {"number": "5.1.1"}}' | nc -l 456; do : ; done ) > "$web_log" &
