...
@test "It should install Kibana 6.0.1" {
...
@test "docker-kibana proxies with credentials to Elasticsearch 6.0" {
  web_log="${BATS_TEST_DIRNAME}/web.log"
  ( while echo "$HTTP_RESPONSE_HEAD" '{"version": {"number": "6.0.1"}}' | nc -l 456; do : ; done ) > "$web_log" &
